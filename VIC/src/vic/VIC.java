...
public class VIC {
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException, ParseException, InterruptedException {
        ...
        options.addOption("o", "outputfile", true, "--required argument: The prefix of output file which contains the results, the file of results will be as [$$prefix].cancervar");
        ...
        //VIC Other Options
        options.addOption("db", "database_vic", true, "--required argument: <path>The  database location/dir for the CancerVar dataset file");
        ...
        options.addOption("cancer_type", true, "The cancer type, please check the doc for the details of cancer type");
        ...
        if (option.hasOption("db")) {
            ...
            civic_markers = vicdb + "/civic_2017_02.txt";
            ...
            if (!known_file.isFile()) {
                ...
                try (BufferedReader reader_list = new BufferedReader(new FileReader(known_file))) {
                    while ((line3 = reader_list.readLine()) != null) {
                        ...
                        if (cls2.length > 1) {
                            String known_variant = cls2[0] + "#" + cls2[1] + "#" + cls2[2] + "#" + cls2[3] + "#" + cls2[4];
...
//                            user_evidence_dict.put(keys, cls2[4].toUpperCase());
        ...
        if (option.hasOption("s") && !evidence_file.isFile()) {
            System.out.println("Warning: Your specified evidence file " + evidencefile + " is not here,please check the path of your evidence file.");
            System.out.println("Your analysis will begin without your specified evidence.");
        }
        ...
        String[] annovar_outfiles = output_path.list((File dir, String name) -> name.endsWith(oname + "." + buildver + "_multianno.txt"));
        ...
        for (String annovar_outfile : annovar_outfiles) {
            ...
            int sum2 = My_Cancer_var_can(opath + annovar_outfile);
        ...
        if ("vcf_m".equals(inputtype.toLowerCase())) {
            System.out.println("Notice: The VIC for VCF with multiple samples is finished, the output file is as [ " + "annovar_outfile" + "<sample name>.VIC ]");
            sum_sample = 1;
            String[] f = outputfile_path.list((File dir, String name) -> name.endsWith(outfilename + "*." + buildver + "_multianno.txt.VIC"));
            for (String f1 : f) {
        ...
        if (out_annf == 0) {
            ...
            System.out.println("ERROR: The name of annotation result file should be like " + outputfile + "*." + buildver + "__multianno.txt");
        }
    ...
    public static void Read_dataset() throws FileNotFoundException, IOException {
        ...
        try {
            ...
            if (evidence_file.isFile()) {
                String line2;
                BufferedReader reader = new BufferedReader(new FileReader(evidence_file));
                try {
            ...
            try (BufferedReader reader_add_marker = new BufferedReader(new FileReader(add_marker))) {
                ...
                while ((line2 = reader_add_marker.readLine()) != null) {
                    ...
                    if (mut.toLowerCase().contains("any ")) {
                        ...
                        if (mut.equalsIgnoreCase("any insertion")) {
                            mut = "insertion_any";
                        }
                        if (mut.equalsIgnoreCase("any deletion")) {
                            mut = "deletion_any";
                        }
                        if (mut.equalsIgnoreCase("any frameshift")) {
                            mut = "frameshift_any";
                        }
                        if (mut.equalsIgnoreCase("any indel")) {
                            mut = "indel_any";
                        }
                        if (mut.equalsIgnoreCase("any missense")) {
                            mut = "missenese_any";
                        }
                        if (mut.equalsIgnoreCase("any nonsense")) {
                            mut = "nonsense_any";
                        }
                        key = gene + "_" + mut;
                        default_s = "";
...
//6.read the user specified SNP list,the variants will pass the frequency check
...
//7.knownGeneCanonical exon file  # caution the build ver, now it is hg19
        try {
            ...
            System.out.printf("Error: can\\'t read the knownGeneCanonical  file %s", knowgenecanonical);
            System.out.println("Error: Please download it from the source website");
            System.exit(0);
        }
...
//8.OMIM mim_pheno.txt file
        try {
            ...
            System.out.printf("Error: can\\'t read the MIM  file %s", mim_pheno);
            System.out.println("Error: Please download it from the source website");
            System.exit(0);
        }
...
//9.OMIM mim_orpha.txt file
        try {
            ...
            System.out.printf("Error: can\\'t read the MIM  file %s \n", mim_orpha);
            System.out.println("Error: Please download it from the source website");
            System.exit(0);
        }
...
//11.cancer_pathway=%(database_vic)s/cancer_pathway.list
        try {
            ...
            BufferedReader reader_cancer_pathway = new BufferedReader(new FileReader(Cancerpathway));
            String line2;
...
//12.cancers_genes.list
        try {
            ...
            BufferedReader reader_cancers_genes = new BufferedReader(new FileReader(cancersgenes));
            String line2;
...
//13.VIC.cancer.types
        try {
            ...
            BufferedReader reader_cancertype = new BufferedReader(new FileReader(cancertype));
            String line2;
    ...
    public static void Check_Downdb() throws IOException, InterruptedException {
        ...
        String ds = "refGene esp6500siv2_all 1000g2015aug avsnp150 dbnsfp30a clinvar_20170905 exac03 dbscsnv11 dbnsfp31a_interpro ensGene knownGene cosmic84_coding icgc21 gnomad_exome";
        for (String dbs : ds.split(" ")) {
            ...
            if (!dbs.equals("rmsk")) {
                cmd = "perl " + annotatevariation + " -bulidver " + buildver + " -downdb -webfrom annovar " + file_name + " " + database_humandb;

            } else {
                cmd = "perl " + annotatevariation + " -bulidver " + buildver + " -downdb " + file_name + " " + database_humandb;
    ...
    public static void Check_Input() throws IOException, InterruptedException {
        ...
        if (inputtype.toLowerCase().equals("vcf")) {
            if (convert2annovar.isFile()) {
                cmd = "perl " + convert2annovar + " -format vcf4 -filter PASS" + inputfile + " -outfile " + inputfile + ".avinput"; //"-filter pass"@huyue 
                System.out.printf("Warning: Begin to convert your vcf file of %s to AVinput of Annovar ...\n", inputfile);
                System.out.println(cmd);
                Process process = Runtime.getRuntime().exec(cmd);
        ...
        if (inputtype.toLowerCase().equals("vcf_m")) {
            if (convert2annovar.isFile()) {
                cmd = "perl " + convert2annovar + "  -format vcf4 -filter PASS " + inputfile + " --allsample --outfile vcf_m";//"fitler pass" @huyue
                System.out.printf("Warning: Begin to convert your vcf file with multiple samples of %s to AVinput of Annovar with All.raw.highqc.vcf.<samplename>.avinput...\n", inputfile);
                System.out.println("Warning: Please attention that the sample names in VCF file should  contain letters/numners only, otherwise the converting may be failure!");
                System.out.println(cmd);
                Process process = Runtime.getRuntime().exec(cmd);
                System.getProperty(cmd);
                process.waitFor();
    ...
    public static void Check_annovar_result() throws IOException, InterruptedException {
        ...
        if (inputtype.toLowerCase().equals("avinput")) {
            cmd = "perl " + tableannovar + " " + inputfile + " " + database_humandb + " -buildver " + buildver + "  -outfile " + outputfile
                    + " -remove -protocol refGene,ensGene,knownGene,esp6500siv2_all,1000g2015aug_all,exac03,avsnp150,dbnsfp30a,dbscsnv11,"
                    + "dbnsfp31a_interpro,clinvar_20170905,cosmic84_coding,icgc21,gnomad_exome  -operation  g,g,g,f,f,f,f,f,f,f,f,f,f,f  -nastring .  " + annovar_options;
            System.out.println(cmd);
            Process process = Runtime.getRuntime().exec(cmd);
            System.getProperty(cmd);
            process.waitFor();
        ...
        if (inputtype.toLowerCase().equals("vcf")) {
            cmd = "perl " + tableannovar + " " + inputfile + ".avinput " + database_humandb + " -buildver " + buildver + " -remove -out " + outputfile
                    + " -protocol refGene,ensGene,knownGene,esp6500siv2_all,1000g2015aug_all,exac03,avsnp150,dbnsfp30a,dbscsnv11,"
                    + "dbnsfp31a_interpro,clinvar_20170905,cosmic84_coding,icgc21,gnomad_exome  -operation  g,g,g,f,f,f,f,f,f,f,f,f,f,f  -nastring . " + annovar_options;
            System.out.println(cmd);
            Process process = Runtime.getRuntime().exec(cmd);
            System.getProperty(cmd);
        ...
        if (inputtype.toLowerCase().equals("vcf_m")) {
            File outfile = new File(outfilename);
            File outputfile_path = new File(outfile.getAbsoluteFile().getParent());
            String[] f = outputfile_path.list((File dir, String name) -> name.endsWith("vcf_m" + "(.*?).avinput"));
            for (String f1 : f) {
                System.out.printf("INFO: Begin to annotate sample file of %s ....", f1);
                String new_outfile = f1.replaceAll(".avinput", "");
                cmd = "perl " + tableannovar + " " + f1 + " " + database_humandb + " -buildver " + buildver + "-out " + new_outfile
                        + "-remove -protocol  refGene,ensGene,knownGene,esp6500siv2_all,1000g2015aug_all,exac03,avsnp150,dbnsfp30a,dbscsnv11,"
                        + "dbnsfp31a_interpro,clinvar_20170905,cosmic84_coding,icgc21,gnomad_exome  -operation  g,g,g,f,f,f,f,f,f,f,f,f,f,f    -nastring . " + annovar_options;
                System.out.println(cmd);
                Process process = Runtime.getRuntime().exec(cmd);
 ...
 /*
    ...
    public static int Check_Thera(String line, Map Funcanno_flgs) throws FileNotFoundException, IOException {
        ...
        for (String cls0 : line_tmp2_sp) {
            ...
            if (cls0.length() > 1 && line_tmp2_sp.length > 0 && cls0_1.length == 5) {
                ...
                if (exon_func.contains("frameshift") && !exon_func.contains("nonframe")) {
    ...
    public static int Check_PopD(String line, Map Freqs_flgs, Map Funcanno_flgs, Map Allels_flgs, Map lof_genes_dict) {
        ...
        String[] cls = line.split("\t");
        String[] Freqs_4pops = {"1000g2015aug_all", "esp6500siv2_all", "ExAC_ALL", "gnomAD_exome_ALL"};
        int tt = 1;
...
//        
    public static int Check_GermD(String line, Map Funcanno_flgs, Map Allels_flgs, Map lof_genes_dict) {
        int GermD = 0;
        String[] cls = line.split("\t");
        int cli = Integer.parseInt(Funcanno_flgs.get("CLINSIG").toString());
        String line_tmp2 = cls[cli];
        if (!line_tmp2.contains("enign") && line_tmp2.contains("athogenic")) {
            GermD = 2;
        }
        if (!line_tmp2.contains("enign") && line_tmp2.contains("drug")) {
            GermD = 2;
        }
        if (line_tmp2.contains("ikely benign") || line_tmp2.contains("enign")) {
            GermD = 1;
        }
        if (line_tmp2.contains("benign") && line_tmp2.contains("athogenic")) {
            GermD = 0;
        }
        if (line_tmp2.contains("ncertain significance")) {
            GermD = 0;
        }
        return GermD;
    }
    ...
    public static int Check_PreP(String line, Map Funcanno_flgs, Map Allels_flgs, Map lof_genes_dict) {
        ...
        if (cls[ph].equals("P") || cls[ph].equals("D")) {
            dam = dam + 1;
        }
        if (cls[ph].equals("B")) {
            ben = ben + 1;
        ...
        if (cls[taster].equals("A") || cls[taster].equals("D")) {
            dam = dam + 1;
        }
        if (cls[taster].equals("P")) {
            ben = ben + 1;
        }
        if (cls[taster].equals(".")) {
            var = var + 1;
        }
    ...
    public static String Assign(String BP, String line, Map Freqs_flgs, Map Funcanno_flgs, Map Allels_flgs) throws FileNotFoundException, IOException {
        ...
        if (paras.containsKey("evidence_file")) {
            File evidence_file = new File(evidencefile);
            if (evidence_file.isFile()) {
    ...
    public static int My_Cancer_var_can(String _annovarfile) throws FileNotFoundException, IOException {
        ...
        Freqs_flgs.put("gnomAD_exome_ALL", 0);
        Freqs_flgs.put("gnomAD_exome_AFR", 0);
        Freqs_flgs.put("gnomAD_exome_AMR", 0);
        Freqs_flgs.put("gnomAD_exome_ASJ", 0);
        Freqs_flgs.put("gnomAD_exome_EAS", 0);
        Freqs_flgs.put("gnomAD_exome_FIN", 0);
        Freqs_flgs.put("gnomAD_exome_NFE", 0);
        Freqs_flgs.put("gnomAD_exome_OTH", 0);
        Freqs_flgs.put("gnomAD_exome_SAS", 0);
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fw))) {
            ...
            if (paras.get("otherinfo").toString().toLowerCase().contains("true")) {
                writer.write("Chr\t" + "Start\t" + "End\t" + "Ref\t" + "Alt\t" + "Ref.Gene\t" + "Func.refGene\t" + "ExonicFunc.refGene\t"
                        ...
                        + "Freq_ExAC_ALL\t" + "Freq_esp6500siv2_all\t" + "Freq_1000g2015aug_all\t" + "Freq_gnomAD_ALL\t" + "FATHMM_pred\t" + "CADD_phred\t"
...
//                    System.out.println("line in strs@1786 is: " + line);
                    ...
                    if (line_sum == 0) {
                        ...
                        int intclinsig = Integer.parseInt(Funcanno_flgs.get("CLINSIG").toString());
                        ...
                        String Freq_gnomAD_exome_POPs = "AFR:" + cls[Freqs_flgs.get("gnomAD_exome_AFR")] + ",AMR:" + cls[Freqs_flgs.get("gnomAD_exome_AMR")]
                                + ",ASJ" + cls[Freqs_flgs.get("gnomAD_exome_ASJ")] + ",EAS" + cls[Freqs_flgs.get("gnomAD_exome_EAS")]
                                + ",FIN" + cls[Freqs_flgs.get("gnomAD_exome_FIN")] + ",NFE:" + cls[Freqs_flgs.get("gnomAD_exome_NFE")]
                                + ",SAS:" + cls[Freqs_flgs.get("gnomAD_exome_SAS")] + ",OTH:" + cls[Freqs_flgs.get("gnomAD_exome_OTH")];
                        String OMIM = ".";
                        String pheno_MIM;
                        ...
                        if (cls.length > Funcanno_flgs.get("Otherinfo")) {
                            writer.write(cls[Allels_flgs.get("Chr")] + "\t" + cls[Allels_flgs.get("Start")] + "\t"
                                    ...
                                    + cls[Freqs_flgs.get("1000g2015aug_all")] + "\t" + cls[Freqs_flgs.get("gnomAD_exome_ALL")] + "\t" + cls[Funcanno_flgs.get("FATHMM_pred")] + "\t"
                                    ...
                                    + Freq_ExAC_POPs + "\t" + Freq_gnomAD_exome_POPs + "\t" + OMIM + "\t" + pheno_MIM + "\t" + orpha_s + "\t" + orpha_details + "\t" + cls[Funcanno_flgs.get("Otherinfo")] + "\n");
                        } else {
                            writer.write(cls[Allels_flgs.get("Chr")] + "\t" + cls[Allels_flgs.get("Start")] + "\t"
                                    + cls[Allels_flgs.get("End")] + "\t" + cls[Allels_flgs.get("Ref")] + "\t" + cls[Allels_flgs.get("Alt")] + "\t"
                                    + cls[Funcanno_flgs.get("Gene")] + "\t" + cls[Funcanno_flgs.get("Func.refGene")] + "\t"
                                    ...
                                    + cls[Funcanno_flgs.get("dbscSNV_ADA_SCORE")] + "\t" + cls[Funcanno_flgs.get("dbscSNV_RF_SCORE")] + "\t"
                                    + cls[Funcanno_flgs.get("Interpro_domain")] + "\t" + cls[Funcanno_flgs.get("MetaSVM_score")] + "\t" + cls[Funcanno_flgs.get("MutationTaster_pred")] + "\t"
                                    + Freq_ExAC_POPs + "\t" + Freq_gnomAD_exome_POPs + "\t" + OMIM + "\t" + pheno_MIM + "\t" + orpha_s + "\t" + orpha_details + "\n");
                        }
